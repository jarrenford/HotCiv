...
class HotCiv:
    ...
    def getTileAt(self, pos):
        ...
        if tile.getTileType() in [OCEAN, MOUNTAIN, PLAINS, HILLS, FOREST]:
            return tile
        else:
            return False
...
class Tile:
    ...
    def __init__(self, tileType):
        ...
        are: PLAINS, OCEAN, HILLS, MOUNTAIN, and FOREST"""
        ...
        self._production = {FOREST:3, MOUNTAIN:1, HILLS:2,
                            PLAINS:0, OCEAN:0}[tileType]
        
    def isPassable(self):
        """Returns whether or not a Unit can pass through the Tile"""
        
        if self._tileType in [MOUNTAIN, OCEAN]:
            return False
...
def SimpleMap(unitCreateStrategy):
    ...
    tileBoard[1][0] = Tile(OCEAN)
    tileBoard[0][1] = Tile(HILLS)
    tileBoard[2][2] = Tile(MOUNTAIN)
...
def CompareAttackDefenseStrategy(attackingUnit, defendingUnit, battleground,
    ...
    terrainFactor = {PLAINS:1, HILLS:2, FOREST:2, "city":3}
